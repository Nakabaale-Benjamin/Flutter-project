  ...
  Future<int?> _getAvailableBedspace(String hallId) async {
    try {
      DocumentSnapshot hallDoc =
          await _db.collection('halls').doc(hallId).get();
    ...
    try {
      ...
      for (int i = 0; i < studentIds.length; i++) {
        ...
        try {
          ...
          final governmentOrPrivateStudent =
              studentData['governmentOrPrivateStudent'] as String?;
          ...
          final criteriaAllowedcgpa = _getNumFromMap(criteria, 'allowed_CGPA');
          final criteriaAllowedpoints =
              _getNumFromMap(criteria, 'allowed_UACE_points');
          final criteriaPassmark = _getNumFromMap(criteria, 'passmark');
          // Calculate points based on criteria
          if (cgpa != null &&
              criteriaCgpa != null &&
              criteriaAllowedcgpa != null &&
              cgpa >= criteriaAllowedcgpa) {
          ...
          if (points != null &&
              criteriaAllowedpoints != null &&
              points >= criteriaAllowedpoints) {
            weight += criteriaUace?.toInt() ?? 0;
          }

          // Check if the points exceed the threshold
          if (criteriaPassmark != null && weight >= criteriaPassmark) {
            ...
            if (hallName != null) {
              ...
              if (totalBedspaces != null) {
                // Assign bedspace
                int assignedBedspace =
                    (hallBedspaceAssignments[hallName] ?? 0) + 1;
    ...
    try {
      ...
      return querySnapshot.docs.map((doc) {
        ...
        return {
          ...
          'weight':
              data['weight'] ?? 0, // Default to 0 if 'points' is not present
        };
      }).toList();
    } catch (e) {
      // Log the error
